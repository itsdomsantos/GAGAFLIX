  ...
  return (
    <div className="pb-16 pt-24 sm:pt-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <p className="mt-3 max-w-xl text-muted">Every tour is a show. Step into the setlist.</p>
      </div>

      {/* Desktop: a filmstrip accordion — each tour is a panel that expands on hover. */}
      <div className="mt-10 hidden h-[72vh] gap-1.5 px-4 sm:px-6 md:flex">
        {tours.map((tour) => (
          <ToursPanel key={tour.slug} tour={tour} />
        ))}
      ...
      <div className="mt-8 flex flex-col gap-2 px-4 sm:px-6 md:hidden">
        {tours.map((tour) => (
          <ToursBanner key={tour.slug} tour={tour} />
        ))}
        {tours.length === 0 && <EmptyState />}
      </div>
    </div>
...
function ToursPanel({ tour }: { tour: Tour }) {
  return (
    <Link
    ...
    >
      ...
      {/* Collapsed state: the tour name written up the spine. */}
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col justify-end p-6 opacity-0 transition-opacity duration-500 delay-150 group-hover/p:opacity-100">
        ...
        {tour.logo_url ? (
          ...
          <img src={tour.logo_url} alt={tour.name} className="mt-2 max-h-20 w-auto self-start object-contain" />
        ) : (
          <h2 className="font-display mt-1 text-4xl [text-wrap:balance] lg:text-5xl" style={{ color: tour.accent }}>
            {tour.name}
          </h2>
        )}
        {tour.tagline && <p className="mt-2 text-sm italic text-text/90">{tour.tagline}</p>}
        <p className="mt-2 line-clamp-3 max-w-md text-sm text-text/80">{tour.description}</p>
        <span
...
function ToursBanner({ tour }: { tour: Tour }) {
  return (
    <Link
      href={`/tours/${tour.slug}`}
    ...
    >
      ...
      <div className="relative w-full p-4">
        <p className="text-[11px] font-semibold uppercase tracking-[0.25em]" style={{ color: tour.accent }}>
          Tour · {tour.years}
        </p>
        {tour.logo_url ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={tour.logo_url} alt={tour.name} className="mt-1 max-h-12 w-auto object-contain" />
        ) : (
          <h2 className="font-display mt-0.5 text-3xl" style={{ color: tour.accent }}>
            {tour.name}
          </h2>
        )}
        {tour.tagline && <p className="mt-1 line-clamp-1 text-xs text-text/70">{tour.tagline}</p>}
      </div>
    </Link>
  );
}

/** Shared background: the tour art (grayscale, revealed in colour on hover) + accent wash. */
function Backdrop({ tour }: { tour: Tour }) {
  const art = tour.backdrop_url ?? tour.poster_url;
  return (
    <>
      {art ? (
        // eslint-disable-next-line @next/next/no-img-element
        ...
        <span
          ...
          style={{ background: `radial-gradient(120% 90% at 50% 0%, ${tour.accent}44, var(--surface) 78%)` }}
        />
      )}
      {/* readability wash + accent edge */}
      <span className="absolute inset-0 -z-10 bg-gradient-to-t from-bg via-bg/40 to-bg/10" />
      <span className="absolute inset-x-0 top-0 h-1" style={{ background: tour.accent }} />
    </>
  );
}

function EmptyState() {
  return <p className="text-muted">No tours yet — coming soon, Little Monster. 🐾</p>;
}
